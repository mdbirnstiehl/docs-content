...
* The [`securitySolution:enableAlertsAndAttacksAlignment`](kibana://reference/advanced-settings.md#kibana-siem-settings) advanced setting turned on (only required for {{stack}} 9.4). 
* The ``securitySolution:enableAttackDiscoveryWorkflows` advanced setting turned on if you want the settings flyout with skill, query, and workflow retrieval.
