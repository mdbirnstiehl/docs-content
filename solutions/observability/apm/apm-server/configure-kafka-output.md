...
:::{admonition} Kafka timestamps and APM Server
...
[log.message.timestamp.after.max.ms](https://docs.confluent.io/platform/current/installation/configuration/topic-configs.html#message-timestamp-after-max-ms).
  ...
  These properties limit the time difference between the message timestamp (from APM Server) and the Kafka broker receive time.
...
* For Kafka version 0.10.0.0+ the message creation timestamp is set by APM Server and equals the initial timestamp of the event. This behavior affects the retention policy in Kafka. For example, if an APM Server event was created 2 weeks ago, the retention policy is set to 7 days and the message from APM Server arrives to Kafka today, it is immediately discarded because the timestamp value is before the last 7 days.
...
To use this output, edit the APM Server configuration file to deactivate the {{es}} output by commenting it out, and enable the Kafka output by uncommenting the Kafka section.
