...
4. {applies_to}`stack: preview 9.2` Choose whether {{agents}} using this output should send data to [wired streams](/explore-analyze/streams/streams.md#streams-wired-streams). Using this feature requires additional steps. For more details, refer to [Ship data to streams → {{fleet}}](/explore-analyze/streams/wired-streams.md#streams-wired-streams-ship).
:::::
