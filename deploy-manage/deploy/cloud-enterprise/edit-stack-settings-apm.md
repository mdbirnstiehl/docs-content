...
2. Change the {{es}} hosts where {{agent}}s send data from the default public URL, to the internal URL. In {{kib}}, navigate to **Fleet** and select the **{{ecloud}} agent policy**. Click **Fleet settings** and update the {{es}} hosts URL. For example, if the current URL is `https://<CLUSTER_ID>.us-central1.gcp.foundit.no:9244`, change it to `http://<CLUSTER_ID>.containerhost:9244`.
