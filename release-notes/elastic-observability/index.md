...
* Adds a continuous Knowledge Indicator extraction workflow to Streams **Significant Events Discovery** that automatically identifies stream features on a configurable schedule; users can configure the interval and exclusion patterns in the **Settings** tab [#260322]({{kib-pull}}260322).
* Enables users to iteratively refine AI-generated partition suggestions in Streams by providing natural language guidance, with context preserved across refinement rounds [#260264]({{kib-pull}}260264).
...
* Enables **Unified Rules** as the default experience, merging Observability rules and Stack Management rules into a single interface; existing links to legacy rule experiences redirect automatically [#258214]({{kib-pull}}258214).
...
* Adds cluster-wide SLO health scanning via API-triggered background tasks; results are paginated and stored in a data stream with 7-day retention [#248004]({{kib-pull}}248004).
...
* Enables the instance selector on the **SLO Details** page when browsing grouped SLOs without a specific instance selected, allowing users to search and navigate to individual instances [#247638]({{kib-pull}}247638).
...
* Removes the confirmation modal when switching to AI Agent mode; the transition now takes effect immediately [#264839]({{kib-pull}}264839).
...
* Adds AI Agent as the default Observability chat experience and displays a one-time, per-space announcement modal; users can return to the classic AI Assistant via **GenAI Settings** [#260570]({{kib-pull}}260570).
...
* Adds table borders to metadata and dimensions tables in the **View details** flyout [#252329]({{kib-pull}}252329).
* Adds OTel and semconv support for k8s pods, k8s containers, and Docker containers in the APM **Infrastructure** tab [#252188]({{kib-pull}}252188).
...
* Removes the **Try the new AI Agent** opt-in popover from the AI Assistant chat header and settings menu [#264263]({{kib-pull}}264263).
* Removes beta labels and badges from the AI Agent chat experience [#264200]({{kib-pull}}264200).
